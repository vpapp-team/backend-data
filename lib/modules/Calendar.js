...
main.update = calendarRefs => new Promise((resolve, reject) => {
  Promise.all(calendarRefs
    ...
    .then(calendars => Promise.all(calendars.map((events, index) => syncCalendar(events, calendars[index].uuid))))
...
const syncCalendar = (events, _masterUUID) => new Promise((resolve, reject) => {
  ...
  const query = CONFIG.sqlPool.query(`SELECT * FROM ${CONFIG.mysql_readwrite.tables.CALENDAR} WHERE \`_masterUUID\` = ? AND \`outdated\` IS NULL`, [events[0]._masterUUID]);
  ...
  query.on('end', () => {
    ...
    return Promise.all([].concat(
      Array.from(removed.keys())
        .map(uuid => UTIL.promisifiedQuery(CONFIG.sqlPool, `UPDATE ${CONFIG.mysql_readwrite.tables.CALENDAR} SET \`outdated\` = ? WHERE \`uuid\` = ? AND \`_masterUUID\` = ?`, [TYPES.get('General').get('Time').now()
          .simplify(), uuid, _masterUUID])),
      events.map(event => UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.CALENDAR} SET ?`, [Object.assign(event.toSQL(), { added: TYPES.get('General').get('Time').now()
        .simplify() })]))
    )).then(() => {
      const now = TYPES.get('General').get('Time').now()
        .simplify();
      return UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.UPDATES} (\`lastUpdate\`, \`category\`) VALUES (?, ?) ON DUPLICATE KEY UPDATE \`lastUpdate\` = ?`, [now, 'calendar', now]);
    }).then(() => resolve(true))
      .catch(reject);
  });
});
