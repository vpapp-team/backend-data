...
main.update = () => new Promise((resolve, reject) => {
  main().then(standins => {
    ...
    const query = CONFIG.sqlPool.query(`SELECT * FROM ${CONFIG.mysql_readwrite.tables.STANDINS} WHERE \`outdated\` IS NULL AND \`day\` IN ('${UTIL.unDoub(standins.map(i => i.day.simplify())).join(`','`)}')`);
    ...
    return query.on('end', () => {
      ...
      return Promise.all([].concat(
        Array.from(removed.keys())
          .map(uuid => UTIL.promisifiedQuery(CONFIG.sqlPool, `UPDATE ${CONFIG.mysql_readwrite.tables.STANDINS} SET \`outdated\` = ? WHERE \`uuid\` = ?`, [TYPES.get('General').get('Time').now()
            .simplify(), uuid])),
        standins.map(standin => UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.STANDINS} SET ?`, [Object.assign(standin.toSQL(), { added: TYPES.get('General').get('Time').now()
          .simplify() })]))
      )).then(() => {
        const now = TYPES.get('General').get('Time').now()
          .simplify();
        return UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.UPDATES} (\`lastUpdate\`, \`category\`) VALUES (?, ?) ON DUPLICATE KEY UPDATE \`lastUpdate\` = ?`, [now, 'stand-in', now]);
      }).then(() => resolve(true))
        .catch(reject);
    });
  }).catch(reject);
