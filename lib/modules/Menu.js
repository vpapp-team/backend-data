...
const parsePage = page => new Promise((resolve, reject) => {
  page.getTextContent({ disableCombineTextItems: true }).then(content => {
    ...
    for (const item of content.items) {
      ...
      if (!rows.some(i => UTIL.betweenNum(item.centerY, i, i, ROW_TOLERANCE))) {
    ...
    const filledRows = sortedRows.map(a =>
      content.items
        .filter(i => UTIL.betweenNum(i.centerY, a, a, ROW_TOLERANCE))
        .map(r => r.str)
        .join('')
        .trim()
    );
...
main.update = () => new Promise((resolve, reject) => {
  main().then(menus => {
    ...
    const query = CONFIG.sqlPool.query(`SELECT * FROM ${CONFIG.mysql_readwrite.tables.MENU} WHERE \`outdated\` IS NULL`);
    ...
    return query.on('end', () => {
      ...
      return Promise.all([].concat(
        Array.from(removed.keys())
          .map(uuid => UTIL.promisifiedQuery(CONFIG.sqlPool, `UPDATE ${CONFIG.mysql_readwrite.tables.MENU} SET \`outdated\` = ? WHERE \`uuid\` = ?`, [TYPES.get('General').get('Time').now.simplify(), uuid])),
        menus.map(menu => UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.MENU} SET ?`, [Object.assign(menu.toSQL(), { added: TYPES.get('General').get('Time').now.simplify() })]))
      )).then(() => {
        const now = TYPES.get('General').get('Time').now.simplify();
        return UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.UPDATES} (\`lastUpdate\`, \`category\`) VALUES (?, ?) ON DUPLICATE KEY UPDATE \`lastUpdate\` = ?`, [now, 'menu', now]);
      }).then(() => resolve(true))
        .catch(reject);
    });
  }).catch(reject);
