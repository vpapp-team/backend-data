...
const parseOutside = (items, headers, indexes, meta) => {
  ...
  const rawOutside = items.filter(i => !UTIL.betweenNum(i.centerX, headers[0].start, headers[headers.length - 1].end) ||
      !UTIL.betweenNum(i.centerY, indexes[0].start, indexes[indexes.length - 1].end));
  ...
  const numbers = rawOutside
    .filter(a => a.str.toLowerCase() === 'nr' && UTIL.betweenNum(topRow, a.centerY, a.centerY, ROW_COLUMN_TOLERANCE))
    .sort((a, b) => a.centerX - b.centerX);
  const tables = numbers.map((number, index) => rawOutside.filter(i => UTIL.betweenNum(i.centerX, number.transform[4], numbers[index + 1] ? numbers[index + 1].transform[4] : Infinity)));
...
const parseOutsideTable = (items, lastNum, topRow, parsed, meta) => {
  ...
  const headers = items.filter(i => UTIL.betweenNum(topRow, i.centerY, i.centerY, ROW_COLUMN_TOLERANCE));
  items = items.filter(i => !UTIL.betweenNum(topRow, i.centerY, i.centerY, ROW_COLUMN_TOLERANCE));

  // Build the rows the items are in
  const rows = [];
  for (const item of items.map(i => i.centerY)) {
    if (!rows.some(r => UTIL.betweenNum(r, item, item, ROW_COLUMN_TOLERANCE))) rows.push(item);
  }
  const rowItems = rows.map(r => items
    .filter(i => UTIL.betweenNum(r, i.centerY, i.centerY, ROW_COLUMN_TOLERANCE))
    .sort((a, b) => a.centerX - b.centerX));
  ...
  for (const row of rowItems) {
    ...
    for (const item of row) {
      let itemHead = headers.find(h => UTIL.betweenNum(item.centerX, h.transform[4], h.transform[4] + h.width, ROW_COLUMN_TOLERANCE));
      if (!itemHead) {
        continue;
...
const parseFields = (items, headers, indexes) => {
  ...
  for (let day = 0; day < headers.length; day++) {
    for (let lesson = 0; lesson < indexes.length; lesson++) {
      ...
      const fieldItems = items
        .filter(item => UTIL.betweenNum(item.centerX, bounds.lb, bounds.rb) &&
          UTIL.betweenNum(item.centerY, bounds.tb, bounds.bb));
      fields.push({
        day,
        lesson,
        bounds,
  ...
  for (const field of fields) {
    ...
    if (UTIL.betweenNum(field.items[0].centerX, middle, field.bounds.rb)) {
      // Split lesson with at least one right
      if (field.items.some(item => UTIL.betweenNum(item.centerX, middle, field.bounds.lb))) {
        // Also has some right
        let leftItems = field.items
          .filter(item => UTIL.betweenNum(item.centerX, field.bounds.lb, middle) &&
            UTIL.betweenNum(item.centerY, field.bounds.tb, field.bounds.bb));
      ...
      field.items = field.items
        .filter(item => UTIL.betweenNum(item.centerX, middle, field.bounds.rb) &&
          UTIL.betweenNum(item.centerY, field.bounds.tb, field.bounds.bb));
      field.averageY = buildAverage(field.items, 'centerY');
      field.bounds.lb = middle;
      field.orientation = 'even';
    } else if (!field.items.some(item => UTIL.betweenNum(item.centerX, middle, field.bounds.rb))) {
...
const parseNewLessonItem = (field, outside, meta) => {
  ...
  if (replace) {
    ...
    if (!outside.has(replacer)) throw new Error(`unknown replace: "${replace.input}" for class: ${meta.class.simplify()} on day: "${field.day + 1}" hour: "${field.lesson + 1}"`);
...
const parseOldLessonItem = (field, meta) => {
  ...
  const columns = [];
  for (const item of field.items) {
    if (!columns.some(i => UTIL.betweenNum(item.centerX, i, i, ROW_COLUMN_TOLERANCE))) {
  ...
  const rows = [];
  for (const item of field.items) {
    if (!rows.some(i => UTIL.betweenNum(item.centerY, i, i, ROW_COLUMN_TOLERANCE))) {
  ...
  for (const row of sortedRows) {
    const items = field.items.filter(item => UTIL.betweenNum(item.centerY, row, row, ROW_COLUMN_TOLERANCE));
    ...
    let buffer = items.find(i => UTIL.betweenNum(i.centerX, columns[0], columns[0], ROW_COLUMN_TOLERANCE));
    if (buffer) infos.subject = buffer.str;
    buffer = items.find(i => UTIL.betweenNum(i.centerX, columns[1], columns[1], ROW_COLUMN_TOLERANCE));
    if (buffer) infos.teacher = buffer.str;
    buffer = items.find(i => UTIL.betweenNum(i.centerX, columns[2], columns[2], ROW_COLUMN_TOLERANCE));
...
main.update = () => new Promise((resolve, reject) => {
  main().then(parsedTimetables => {
    ...
    CONFIG.sqlPool.query(`SELECT * FROM ${CONFIG.mysql_readwrite.tables.TIMETABLE} WHERE \`outdated\` IS NULL`, (err, rows) => {
      ...
      return Promise.all([].concat(
        removeDB.map(uuid => UTIL.promisifiedQuery(CONFIG.sqlPool, `UPDATE ${CONFIG.mysql_readwrite.tables.TIMETABLE} SET \`outdated\` = ? WHERE \`uuid\` = ?`, [TYPES.get('General').get('Time').now.simplify(), uuid])),
        addDB.map(timetable => UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.TIMETABLE} SET ?`, [Object.assign(timetable.toSQL(), { added: TYPES.get('General').get('Time').now.simplify() })]))
      )).then(() => {
        const now = TYPES.get('General').get('Time').now()
          .simplify();
        return UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.UPDATES} (\`lastUpdate\`, \`category\`) VALUES (?, ?) ON DUPLICATE KEY UPDATE \`lastUpdate\` = ?`, [now, 'timetables', now]);
      }).then(() => resolve(true))
        .catch(reject);
    });
  }).catch(reject);
