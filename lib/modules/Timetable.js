...
const parsePage = pageContent => {
  ...
  return new (TYPES.get('Data').get('Timetable'))({
    uuid: meta.uuid,
    type: 0,
    master: meta.class,
...
const parseMeta = items => {
  ...
  return {
    ...
    class: new (TYPES.get('General').get('ClassDiscriminator'))(`${Number(classMatch[1])}${classMatch[2] || ''}`),
    activation: new (TYPES.get('General').get('Time'))({
...
const parseOutside = (items, headers, indexes, meta) => {
  ...
  const rawOutside = items.filter(i => !i.centerX.betweenNum(headers[0].start, headers[headers.length - 1].end) ||
      !i.centerY.betweenNum(indexes[0].start, indexes[indexes.length - 1].end));
  // Split the items into the individual tables
  const topRow = rawOutside.map(x => x.centerY).reduce((a, b) => a > b ? a : b, null);
  const numbers = rawOutside
    .filter(a => a.str.toLowerCase() === 'nr' && topRow.betweenNum(a.centerY, a.centerY, ROW_COLUMN_TOLERANCE))
    .sort((a, b) => a.centerX - b.centerX);
  const tables = numbers.map((number, index) =>
    rawOutside.filter(i =>
      i.centerX.betweenNum(number.transform[4], numbers[index + 1] ? numbers[index + 1].transform[4] : Infinity)
    )
  );
...
const parseOutsideTable = (items, lastNum, topRow, parsed, meta) => {
  ...
  const headers = items.filter(i => topRow.betweenNum(i.centerY, i.centerY, ROW_COLUMN_TOLERANCE));
  items = items.filter(i => !topRow.betweenNum(i.centerY, i.centerY, ROW_COLUMN_TOLERANCE));

  // Build the rows the items are in
  const rows = [];
  for (const item of items.map(i => i.centerY)) {
    if (!rows.some(r => r.betweenNum(item, item, ROW_COLUMN_TOLERANCE))) rows.push(item);
  }
  const rowItems = rows.map(r => items
    .filter(i => r.betweenNum(i.centerY, i.centerY, ROW_COLUMN_TOLERANCE))
    .sort((a, b) => a.centerX - b.centerX));
  ...
  for (const row of rowItems) {
    ...
    for (const item of row) {
      let itemHead = headers.find(h =>
        item.centerX.betweenNum(h.transform[4], h.transform[4] + h.width, ROW_COLUMN_TOLERANCE)
      );
      if (!itemHead) {
        continue;
      } else if (itemHead.str.toLowerCase() === 'nr') {
...
const parseFields = (items, headers, indexes) => {
  ...
  for (let day = 0; day < headers.length; day++) {
    for (let lesson = 0; lesson < indexes.length; lesson++) {
      ...
      const fieldItems = items
        .filter(item => item.centerX.betweenNum(bounds.lb, bounds.rb) &&
          item.centerY.betweenNum(bounds.tb, bounds.bb));
      fields.push({
        day,
        lesson,
        bounds,
  ...
  for (const field of fields) {
    ...
    if (offset > MAX_VERTICAL_OFFSET) {
      ...
      // Both increased by 1 to make it human readable (+0 since its prev field)
      if (!prevField) throw new Error(`unknown prevField day: ${field.day + 1} lesson: ${field.lesson}`);
      field.averageY = null;
      ...
      // Both increased by 1 to make it human readable (+2 since its next field)
      if (!nextField) throw new Error(`unknown nextField day: ${field.day + 1} lesson: ${field.lesson + 2}`);
  ...
  for (const field of fields) {
    ...
    if (field.items[0].centerX.betweenNum(middle, field.bounds.rb)) {
      // Split lesson with at least one right
      if (field.items.some(item => item.centerX.betweenNum(middle, field.bounds.lb))) {
        // Also has some right
        let leftItems = field.items
          .filter(item => item.centerX.betweenNum(field.bounds.lb, middle) &&
            item.centerY.betweenNum(field.bounds.tb, field.bounds.bb));
      ...
      field.items = field.items
        .filter(item => item.centerX.betweenNum(middle, field.bounds.rb) &&
          item.centerY.betweenNum(field.bounds.tb, field.bounds.bb));
      field.averageY = buildAverage(field.items, 'centerY');
      field.bounds.lb = middle;
      field.orientation = 'even';
    } else if (!field.items.some(item => item.centerX.betweenNum(middle, field.bounds.rb))) {
...
const parseNewLessonItem = (field, outside, meta) => {
  ...
  if (replace) {
    ...
    // Both increased by 1 to make it human readable
    if (!outside.has(replacer)) throw new Error(`unknown replace: "${replace.input}" for class: ${meta.class.simplify()} on day: "${field.day + 1}" hour: "${field.lesson + 1}"`); // eslint-disable-line max-len
    return outside.get(replacer).map(item => new (TYPES.get('Data').get('Lesson'))({
    ...
    return [new (TYPES.get('Data').get('Lesson'))({
      masterUUID: meta.uuid,
      class: meta.class,
      weekday: field.day,
      lesson: `${field.lesson + 1}${field.height !== 1 ? `-${field.lesson + field.height}` : ''}`,
...
const parseOldLessonItem = (field, meta) => {
  ...
  const columns = [];
  for (const item of field.items) {
    if (!columns.some(i => item.centerX.betweenNum(i, i, ROW_COLUMN_TOLERANCE))) {
  ...
  const rows = [];
  for (const item of field.items) {
    if (!rows.some(i => item.centerY.betweenNum(i, i, ROW_COLUMN_TOLERANCE))) {
  ...
  for (const row of sortedRows) {
    const items = field.items.filter(item => item.centerY.betweenNum(row, row, ROW_COLUMN_TOLERANCE));
    ...
    let buffer = items.find(i => i.centerX.betweenNum(columns[0], columns[0], ROW_COLUMN_TOLERANCE));
    if (buffer) infos.subject = buffer.str;
    buffer = items.find(i => i.centerX.betweenNum(columns[1], columns[1], ROW_COLUMN_TOLERANCE));
    if (buffer) infos.teacher = buffer.str;
    buffer = items.find(i => i.centerX.betweenNum(columns[2], columns[2], ROW_COLUMN_TOLERANCE));
    if (buffer) infos.room = buffer.str;
    if (!infos.subject || !infos.teacher) continue;
    parsed.push(new (TYPES.get('Data').get('Lesson'))({
      masterUUID: meta.uuid,
      class: meta.class,
      weekday: field.day,
      lesson: `${field.lesson + 1}${field.height !== 1 ? `-${field.lesson + field.height}` : ''}`,
...
const buildTeacherTimetables = classTimetables => {
  ...
  for (const teacher of teachers) {
    ...
    timetables.push(new (TYPES.get('Data').get('Timetable'))({
      uuid: TYPES.get('General').get('UUID').new(),
      type: 1,
      master: teacher,
...
main.update = () => new Promise((resolve, reject) => {
  main().then(parsedTimetables => {
    LOGGER.log(`Timetable pulled: ${parsedTimetables.length}`);
    CONFIG.sqlPool.query(`SELECT * FROM ${CONFIG.mysql_readwrite.tables.TIMETABLE} WHERE \`outdated\` IS NULL`, (err, rows) => { // eslint-disable-line max-len
      if (err) return reject(err);
      const databaseTimetables = rows.map(row => new (TYPES.get('Data').get('Timetable'))(row));
      const removeDB = [];
      const addDB = [];
      for (const dbTimetable of databaseTimetables) {
        if (!parsedTimetables.some(pTimetable => pTimetable.sameMaster(dbTimetable))) {
          removeDB.push(dbTimetable.uuid.simplify());
        }
      }
      for (const pTimetable of parsedTimetables) {
        const sameTimetable = databaseTimetables
          .filter(a => a.sameMaster(pTimetable));
        ...
        if (pTimetable.activation.rawNumber <= Math.floor(Date.now() / 24 / 60 / 60 / 1000)) {
          ...
          sameTimetable
            .filter(dbTimetable => dbTimetable.activation.rawNumber > Math.floor(Date.now() / (24 * 60 * 60 * 1000)) &&
                dbTimetable !== isInThere)
            .forEach(table => removeDB.push(table.uuid.simplify()));
          ...
          if (newest) {
            sameTimetable
              .filter(dbTimetable =>
                dbTimetable.activation.rawNumber <= newest.activation.rawNumber && dbTimetable !== newest
              )
              .forEach(table => removeDB.push(table.uuid.simplify()));
          }
      ...
      LOGGER.log(`Timetable new: ${addDB.length} removed: ${removeDB.length}`);
      if (!addDB.length && !removeDB.length) return resolve(false);
      return Promise.all([].concat(
        removeDB.map(uuid => UTIL.promisifiedQuery(CONFIG.sqlPool, `UPDATE ${CONFIG.mysql_readwrite.tables.TIMETABLE} SET \`outdated\` = ? WHERE \`uuid\` = ?`, [TYPES.get('General').get('Time').simpleNow(), uuid])), // eslint-disable-line max-len
        addDB.map(timetable => UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.TIMETABLE} SET ?`, [Object.assign(timetable.toSQL(), { added: TYPES.get('General').get('Time').simpleNow() })])) // eslint-disable-line max-len
      )).then(() => {
        const now = TYPES.get('General').get('Time').simpleNow();
        return UTIL.promisifiedQuery(CONFIG.sqlPool, `INSERT INTO ${CONFIG.mysql_readwrite.tables.UPDATES} (\`lastUpdate\`, \`category\`) VALUES (?, ?) ON DUPLICATE KEY UPDATE \`lastUpdate\` = ?`, [now, 'timetables', now]); // eslint-disable-line max-len
      }).then(() => resolve(true))
        .catch(reject);
    });
  }).catch(reject);
